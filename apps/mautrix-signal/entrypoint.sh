...
init_config() {
    if [[ ! -f "${CONFIG_FILE}" ]]; then
        ...
        if ! /usr/bin/mautrix-signal -c "${CONFIG_FILE}" -e; then
...
init_registration() {
    if [[ ! -f "${REGISTRATION_FILE}" ]]; then
        ...
        if ! /usr/bin/mautrix-signal -g -c "${CONFIG_FILE}" -r "${REGISTRATION_FILE}"; then
...
main() {
    log_info "Starting mautrix-signal bridge"
    ...
    /usr/bin/mautrix-signal -c "${CONFIG_FILE}" --no-update &
    BRIDGE_PID=$!
