...
REGISTRATION_FILE="${HOME}/registration.yaml"
...
init_config() {
    if [[ ! -f "${CONFIG_FILE}" ]]; then
        ...
        /usr/bin/mautrix-linkedin -c "${CONFIG_FILE}" -e || {
...
init_registration() {
    if [[ ! -f "${REGISTRATION_FILE}" ]]; then
        ...
        if ! /usr/bin/mautrix-linkedin -g -c "${CONFIG_FILE}" -r "${REGISTRATION_FILE}"; then
...
main() {
    log_info "Starting mautrix-linkedin bridge"
    ...
    if [[ ! -w "${HOME}" ]]; then
        log_error "Data directory ${HOME} is not writable"
        exit 1
    fi
    ...
    exec /usr/bin/mautrix-linkedin -c "${CONFIG_FILE}" &
    BRIDGE_PID=$!
