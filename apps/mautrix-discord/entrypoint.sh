...
init_config() {
    if [[ ! -f "${CONFIG_FILE}" ]]; then
        ...
        if ! /usr/bin/mautrix-discord -c "${CONFIG_FILE}" -e; then
...
init_registration() {
    if [[ ! -f "${REGISTRATION_FILE}" ]]; then
        ...
        if ! /usr/bin/mautrix-discord -g -c "${CONFIG_FILE}" -r "${REGISTRATION_FILE}"; then
...
main() {
    log_info "Starting mautrix-discord bridge"
    ...
    /usr/bin/mautrix-discord -c "${CONFIG_FILE}" --no-update &
    BRIDGE_PID=$!
